/* ~~~~~~~~~~~~~
	Специальная версия для LISTENER 2
	Поддерживает SIM800 и SIM900r.
	Для работы необходим webserver_listener2 
...
***********	портирование для SIM800
	...
	07.07.2016 - Версия для Listener2, использовать с WebserverListener2.
~~~~~~~~~~~~~*/
...
// ~~~~~~~~~~~
//FIFO
#define GSM_RX_FIFO_SIZE 100		// при условиях до 5мс ScanCycle и GPRS_In_MaxSz =< 50
...
inline static void GSM_Auto(){
	...
	switch(GSM_State){
		...
		case GSM_PowerOn:
			...
			StartTimer32(TD_1hour_resetGPRS_timer, 360000); //Раз в час разъединение и соединение
			StartTimer16(TD_CheckModem,6000);// Проверка уровня сигнала раз в 5 минут	
			GSM_PWRCNTRL_ON();
			InitFIFO();
			counter=0;
...
///------------------------Для СМС/Интернет (пока не задействовано)
		...
		//---В это состояние приходим перед идентификацией
			#ifdef ERGOMERA
			if(counter==0){
		...
		case GSM_ProtocolMode:
			...
			while(GSM_RX_FIFO_Begin != GSM_RX_FIFO_End){	
					...
					if(GSM_RxCharN >= 4){
						...
						if(!strncmp_P(GSM_RxStr, PSTR("CLOS"), 4)) {AppProtocol = _HTTP;GSM_State = GSM_ReStart1;break;}//разорвано соединение
						if(!strncmp_P(GSM_RxStr, PSTR("+PDP"), 4)) {AppProtocol = _HTTP;GSM_State = GSM_ReStart1;break;}//GPRS отключен
						if( (GSM_RxStr[0]==0) && (GSM_RxStr[1]==0)){AppProtocol = _HTTP;GSM_State = GSM_ReStart1;break;}//питание модема пропало
						//GSM_State = GSM_DataMode;
						
						if(AppProtocol == 0){
							InitFIFO();//если попал "мусор", очищаем
							break;
						}
...
/*					if(!IsWebSession()){					
							...
							if(Timer32Stopp(TD_GSM_Reset)){//------ ресет модема раз в сутки
								GSM_State = GSM_ReStart1;
								break;
							}
